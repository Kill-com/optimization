...
class IConteiner{
    protected:
    struct BaseHolder {
        virtual ~BaseHolder() = default;                        // Виртуальный деструктор
        virtual std::unique_ptr<BaseHolder> clone() const = 0;  // Клонирование
        virtual const std::type_info& type() const = 0;         
    };
    ~IConteiner()=default;
};

class Conteiner: protected IConteiner{
    protected:
    template<typename T>
    struct Holder : BaseHolder {
        T value;  // ← Здесь хранится реальное значение!
        explicit Holder(T val) : value(std::move(val)) {}
        // Реализуем все виртуальные методы
...
class Collect: protected Conteiner{
    private:
    std::unique_ptr<BaseHolder> holder;
    protected:
    void collect(std::string path_module){
        auto module = PluginLoader::loadLibrary(path_module);
