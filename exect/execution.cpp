...
void EXECUTER::exect(){
    ...
    if (!method && !function) {
        ...
        //обработчик принимает функцию и два float, возвращает float
        ...
        //функция принимает float и возвращает float
          ...
          if (!process && !process_f) {
            std::cerr << "Function 'f' not found!" << std::endl;
                ...
                std::cout<<process(process_f,1,2)<<std::endl; //вызываем метод
            }
    }
}
